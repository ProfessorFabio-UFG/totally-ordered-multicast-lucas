...
import pickle
from requests import get
from constMP import *  # portas base e constantes
import sys
...
# Ajusta portas base com offset peer_id para evitar conflitos
...
def sendLogs():
    ...
    for i in range(NUM_MESSAGES):
        lamportClock += 1
        log.append((lamportClock, f"Message {i} from peer {myself}"))
    clientSock = socket(AF_INET, SOCK_STREAM)
    clientSock.connect((SERVER_ADDR, SERVER_PORT))
...
def main():
    ...
    for i in range(NUM_MESSAGES):
        sendMessage(f"Message {i} from peer {myself}", "127.0.0.1")
